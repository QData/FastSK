...
command = [GKM_KERNEL, "-a", str(2), "-l", str(g), "-k", str(k), "-d", str(m), "-R"]
...
command = [GKM_TRAIN, kernel_file, train_pos_file, train_neg_file, svm_file_prefix]
print(" ".join(command))
output = subprocess.check_output(command)
...
# get pos preds
command = [GKM_CLASSIFY, "-l", str(g), "-k", str(k), "-d", str(m), "-R"]
if args.dict is not None:
    command += ["-A", args.dict]
