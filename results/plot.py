...
def auc_summary_fig(csv="auc_results/acc_auc_summary_results.csv"):
    ...
    axes[0].scatter(
        gkm_exact_dna, fastsk_approx_dna, color="r", marker="o", label="gkmSVM-2.0"
    )
    ...
    axes[0].scatter(
        charcnn_dna, fastsk_approx_dna, color="orange", marker="*", label="Char-CNN"
    )
    ...
    axes[1].scatter(
        gkm_exact_prot, fastsk_approx_prot, color="r", marker="o", label="gkmSVM-2.0"
    )
    ...
    axes[1].scatter(
        charcnn_prot, fastsk_approx_prot, color="orange", marker="*", label="Char-CNN"
    )
    ...
    axes[2].scatter(
        ...
        color="c",
        marker="^",
        label="Blended Spectrum Kernel",
    )
    axes[2].scatter(lstm_nlp, fastsk_approx_nlp, color="r", marker="o", label="LSTM")
    axes[2].scatter(
        charcnn_nlp, fastsk_approx_nlp, color="orange", marker="*", label="Char-CNN"
    )
...
def vary_I(csv, dataset):
    ...
    fig, ax = plt.subplots()
    ax.plot(
        proportions, acc, label="Accuracy", marker="x", color="b", linestyle="dashed"
    )
...
def big_fig_threads(csv_dir, outfile):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            ax.plot(
                threads, i50, label="FastSK-Approx (1 thread)", color="g", marker="x"
            )
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            fastsk_approx_no_var = [
                v
                for v in list(df["FastSK-Approx 1 thread no variance 50 iters"])
                if v != 0
            ]
...
def big_fig_increase_g(csv_dir, outfile):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            if dataset != "ZZZ3":
                ax.plot(
                    g, fastsk_approx_conv_auc, label=r"FastSK", marker="x", color="b"
                )
            else:
                ax.plot(
                    g, fastsk_approx_i50_auc, label=r"FastSK", marker="x", color="b"
                )
            ...
            ax.plot(
                g, gkm_approx_auc, label=r"gkmSVM-2.0-Approx", marker="^", color="r"
            )
...
def nlp_perfs(csv):
    ...
    avg_improvement_blended = [
        fast - blend for (fast, blend) in zip(fastsk_auc, blended_auc)
    ]
...
def deep_learning_variance_plot(
    csv="variance_results.csv", outfile="variance_plot.pdf"
):
    df = pd.read_csv(csv)
    ...
    ax.fill_between(
        num_samples, charcnn_lower_ci, charcnn_upper_ci, color="orange", alpha=0.5
    )
...
def approx_vs_exact(csv, outprefix):
    ...
    # convergence
    avg_auc_loss = (
        100 * sum([a - e for a, e in zip(approx_auc, exact_auc)]) / len(exact_auc)
    )
    avg_acc_loss = sum([a - e for a, e in zip(approx_acc, exact_acc)]) / len(exact_acc)
    print(
        "Convergence: AUC loss: {}%, Acc loss: {}%".format(avg_auc_loss, avg_acc_loss)
    )

    # 50 iters
    avg_auc_loss = (
        100 * sum([a - e for a, e in zip(i50_auc, exact_auc)]) / len(exact_auc)
    )
...
def auc_vs_time_fig(d):
    ...
    ax.scatter(fastsk_times, fastsk_aucs, label="FastSK", color="b", marker="x")
    ax.scatter(gkm_times, gkm_aucs, label="gkmSVM-Approx", color="r", marker="o")
...
def speedup_barchart_avg_g(type_, outfile="speedup_barchart_dna.pdf"):
    ...
    for i, d in enumerate(datasets):
        ...
        fastsk_approx = (
            list(df["fastsk_I50"])
            if type_ == "dna"
            else list(df["FastSK-Approx 1 thread"])
        )
        if type_ == "dna":
            gkm_exact = [v for v in list(df["gkm_exact"]) if v != 0]
        else:
        ...
        avg = [
            gkm / fastsk
            for (gkm, fastsk) in zip(gkm_exact, fastsk_approx[: len(gkm_exact)])
        ]
        avg = sum(avg) / len(avg)
        max_ = max(
            [
...
def dna_and_prot_convergence_fig(dna_csv, prot_csv, outfile):
    ...
    for i in range(0, 2):
        ...
        ax2 = ax[i].twinx()
        sd_handle = ax2.plot(
            iters, mean_stdev, label="Kernel variance", marker="o", color="orange"
        )
        ...
        if i == 1:
            ax2.set_ylabel(
                "Kernel Variance", size=axis_font_size, rotation=-90, labelpad=20
            )
        ...
        labels = [
            "{:.1f}% ({})".format(pos, amount) for pos, amount in zip(x_pos, amounts)
        ]
...
def bigfig_stdev_I(csv_dir, outfile):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            acc_handle = ax.plot(
                iters, mean_acc, label="Accuracy", marker="x", color="b"
            )
            ax.fill_between(iters, lower_acc, upper_acc, color="b", alpha=0.5)
            auc_handle = ax.plot(iters, mean_auc, label="AUC", marker="o", color="r")
            ax.fill_between(iters, lower_auc, upper_auc, color="r", alpha=0.5)
