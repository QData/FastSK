...
FastSK can remain robust and reliable thanks to users who notify us of
the problems they encounter. So thank you for [reporting an issue](https://github.com/QData/FastSK/issues).
...
`FastSK`. `git` is not the easiest tool to use but it has the greatest
manual. Type `git --help` in a shell and enjoy. If you prefer books, [Pro
Git](https://git-scm.com/book/en/v2) is a very good reference.

Follow these steps to start contributing:

1. Fork the [repository](https://github.com/QData/FastSK) by
...
4. Set up a development environment by running the following commands in a virtual environment:
   ...
   This will install `FastSK` in editable mode and install `black` and 
   `isort`, packages we use for code formatting.
   
   (If FastSK was already installed in the virtual environment, remove
   it with `pip uninstall FastSK` before reinstalling it in editable
...
5. Develop the features on your branch.
   ...
   `FastSK` relies on `black` and `isort` to format its source code
